package net.motaso.wobble.command;
...
public class ReturnHomeCommand {
    ...
    private int execute(CommandContext<CommandSourceStack> context) {
        ...
        boolean hasHomepos = player.getPersistentData().getIntArray("wobble.homepos").length != 0;

        if (hasHomepos) {
            int[]  playerPos = player.getPersistentData().getIntArray("wobble.homepos");
